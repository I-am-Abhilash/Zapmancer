...
class OpenBiometricsClient(
     ...
     */
    suspend fun getCapabilities(): OpenBiometricsCapabilitiesResponse = try {
     ...
     */
    suspend fun detectFaces(imageBytes: ByteArray): OpenBiometricsDetectResponse = try {
     ...
     */
    suspend fun verifyFaces(idCropBytes: ByteArray, selfieBytes: ByteArray): OpenBiometricsVerifyResponse = try {
        client.submitFormWithBinaryData(
     ...
     */
    suspend fun evaluatePassiveLiveness(imageBytes: ByteArray): OpenBiometricsPassiveLivenessResponse = try {
        client.submitFormWithBinaryData(
     ...
     */
    suspend fun createLivenessSession(preset: String = "eye"): OpenBiometricsLivenessSessionResponse = try {
    ...
    ): OpenBiometricsLivenessEvaluateResponse = try {
        client.submitFormWithBinaryData(
     ...
     */
    suspend fun processDocument(documentBytes: ByteArray): OpenBiometricsDocumentResponse = try {
        client.submitFormWithBinaryData(
     ...
     */
    suspend fun searchWatchlist(faceBytes: ByteArray, threshold: Double = 0.70): OpenBiometricsWatchlistSearchResponse = try {
        client.submitFormWithBinaryData(
     ...
     */
    suspend fun getWatchlists(): List<WatchlistDto> = try {
     ...
     */
    suspend fun createWatchlist(name: String, description: String = ""): WatchlistDto? = try {
     ...
     */
    suspend fun addFaceToWatchlist(watchlistId: String, name: String, faceBytes: ByteArray): Boolean = try {
